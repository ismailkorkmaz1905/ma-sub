...
RUN apt-get update \
    && apt-get install -y --no-install-recommends ca-certificates curl ffmpeg fonts-liberation git openssh-server procps rclone \
...
ENTRYPOINT ["/opt/ma-sub/runpod/container-start.sh"]
