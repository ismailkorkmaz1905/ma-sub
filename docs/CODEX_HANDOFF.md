...
- Episode 11 raw ASR completed on a real RTX 4090 in `986.5 seconds`. The controller reached the Turkish correction handoff after `1,168.031 seconds`. The Turkish correction return later passed. Bounded audio review covered 245 records, resolved 107, and left 138 pending manual acoustic decisions.
...
1. Preserve the accepted Turkish correction return and the CTC report with both recorded hashes. Run `.\mas.ps1 review-audio 11` locally and resolve all 138 pending records with manual acoustic evidence; the CTC probe's 0 strict closures are not decisions. The localhost UI validates the report, correction artifacts, and WAV hashes and atomically saves `review/audio_review_overrides.json` without starting RunPod.
2. Run `.\mas.ps1 run 11` only after all 138 decisions are saved. The controller uploads the override JSON before the pipeline, with byte-count and SHA-256 readback at both staging and final remote paths. Record production forced alignment and Indonesian handoff evidence. The controller starts paid compute only after local preflight passes.
