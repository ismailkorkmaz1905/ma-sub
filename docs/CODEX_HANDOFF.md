...
- RunPod Pod `p54vvbyu76eztn` (`muhtemel-ask-ep12`) was identified with an RTX 4090 configuration and `/workspace` network-volume mount. A dedicated `ma-sub-control` API key is stored outside Git in the Windows user environment. The first Episode 11 attempt on 2026-09-05 failed before SSH because the existing Pod had no public key; the key was corrected and read back. The second attempt completed SSH and dependency installation but failed strict preflight because Windows had written the transferred `runtime.env` with CRLF, adding a carriage return to Linux secret and virtualenv paths. The controller stopped the Pod and REST verified `desiredStatus=EXITED`; the recorded second-run elapsed time was `299.485 seconds`. Runtime environment files are now forced to LF with a regression assertion. Successful strict preflight and GPU execution still require a paid retry.
...
- A third Episode 11 attempt verified SSH, cookie and rclone preflight, then failed before inference because `uv pip sync` had installed only the direct requirement lines and omitted Torch transitive dependencies (`typing_extensions` was the observed missing import). The controller verified `EXITED`; elapsed time was `360.016 seconds`. Bootstrap now reuses the persistent `/workspace/ma-sub/.venv`, keeps uv and model caches under `/workspace`, and uses dependency-resolving `uv pip install`. This correction has not yet been exercised on the paid Pod.
- The next automated retry stopped before installation because uv's default first-index policy selected an incompatible `requests` inventory from the PyTorch index. Both configured indexes are required and trusted for this lock, so bootstrap and Docker now use uv's all-index resolution strategy. A Linux/Python 3.11 resolution of all `131 packages` completed locally; paid-Pod verification remains required.
