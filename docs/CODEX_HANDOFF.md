...
LATEST REPAIR GROUP 2: primary ASR now persists before VAD/rescue with actual model/runtime/producer binding; local Indonesian-return preflight runs before paid startup. See WORKFLOW_REPAIR_PROGRESS.md for tests and limitations. Sol's isolated lease scaffold is not yet approved for paid execution. User authorizes necessary key creation; the live S3 form grants all-volume read/write access, and final-action scope confirmation is pending. No new Pod start or acoustic pilot has occurred. All older authority/budget paragraphs below are historical where they conflict with the active task described next.
