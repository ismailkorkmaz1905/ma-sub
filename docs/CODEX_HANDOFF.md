...
1. Run `.\mas.ps1 run 11`. This is the first non-EP12 candidate and starts paid compute only after local preflight passes.
2. Record real Pod startup, SSH, strict GPU preflight, Docker/runtime dependency, authenticated source acquisition, and external EXITED evidence.
