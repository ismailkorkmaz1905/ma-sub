"""V2 deterministic subtitle segmentation from forced-aligned timed words.
    ...
    for offset, source in enumerate(source_words):
        ...
        if start < previous_end:
            raise SegmentationError(
                "Overlapping word intervals are not canonical alignment: "
                f"offset {offset - 1} ends at {previous_end}, offset {offset} starts at {start}"
            )
        if not text.strip():
