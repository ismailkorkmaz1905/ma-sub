...
class DownloadResumeTests(unittest.TestCase):
    ...
    def test_cookie_reaches_media_and_caption_without_persisting_secret(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch("src.download._import_yt_dlp", return_value=_fake_yt_dlp_module(state)):
    ...
    def test_authenticated_caption_failure_redacts_secret(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("src.download._import_yt_dlp", return_value=_fake_yt_dlp_module(state)),
                self.assertLogs("src.download", level="WARNING") as captured,
            ):
                result = download_source(
                    self.url,
                    source_dir,
    ...
    def test_interrupted_marker_commit_leaves_no_uncommitted_final_source(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch(
                    "src.download._import_yt_dlp",
                    return_value=_fake_yt_dlp_module(state),
                ),
                patch("src.download.write_stage_marker", side_effect=KeyboardInterrupt),
            ):
