...
from src.audio_review_v2 import resolve_tr_audio_reviews_v2
from src.archive_v2 import _load_finalization_inputs
from src.download import atomic_write_json, sha256_file, sha256_json, write_stage_marker
from src.finalize_v2 import FinalizationV2Error, finalize_episode_v2
from src.forced_align import _alignment_sha256, align_corrected_segments
from src.id_translation import (
    build_id_translation_records,
    create_id_translation_output_zip,
)
from src.media import AUDIO_ALIGNMENT_VERSION
from src.raw_asr_v2 import RawASRV2Config
from src.srt import parse_srt
from src.subtitle_qa import SubtitleQAError
from src.timing_qa_v2 import TimingQAV2Error
...
class FinalizeV2Tests(unittest.TestCase):
    ...
    def _workspace(self, temporary: str) -> tuple[Path, dict[str, Path]]:
        ...
        config = project / "SYSTEM_V2_BETA" / "config"
    ...
    def test_pass_rebuilds_full_evidence_and_preserves_legacy_v1(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                "src.finalize_v2.mux_softsubs", side_effect=self._fake_mux
            ) as mocked:
    ...
    def test_weakened_alignment_policy_or_hard_counter_cannot_reach_mux(
        ...
        for label, mutate in mutations.items():
            with self.subTest(label=label), tempfile.TemporaryDirectory() as temporary:
                ...
                with patch("src.finalize_v2.mux_softsubs") as mocked:
    ...
    def test_incomplete_mux_verification_publishes_no_pass(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch("src.finalize_v2.mux_softsubs", side_effect=bad_mux):
                with self.assertRaisesRegex(
    ...
    def test_post_mux_evidence_mutation_rolls_back_publication(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with patch(
                "src.finalize_v2.mux_softsubs", side_effect=mutating_mux
            ):
                with self.assertRaisesRegex(
    ...
    def test_report_commit_failure_restores_all_prior_v2_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch(
                    "src.finalize_v2.mux_softsubs",
                    side_effect=self._fake_mux,
                ),
                patch(
                    "src.finalize_v2.atomic_write_json",
