...
from src.download import sha256_file, write_stage_marker
import src.raw_asr_v2 as raw_asr_v2_module
from src.raw_asr_v2 import (
    RawASRV2Config,
...
from src.speech_coverage import analyze_speech_coverage
...
class RawASRV2Tests(unittest.TestCase):
    def test_explicit_review_budget_does_not_relax_automatic_detector_bound(self) -> None:
        ...
        with self.assertRaisesRegex(
            TranscriptionError, "explicit_only_candidates=129"
        ):
            raw_asr_v2_module._require_asr_hallucination_candidate_budget(
                automatic_count=0,
                explicit_only_count=129,
                total_count=129,
                reviewable_count=500,
                candidate_reason_counts={},
            )
    ...
    def test_unresolved_speech_becomes_explicit_hole_record(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            source_audio.write_bytes(b"synthetic-audio-source")
            with patch(
                "src.raw_asr_v2._extract_clip", side_effect=_fake_extract_clip
            ):
