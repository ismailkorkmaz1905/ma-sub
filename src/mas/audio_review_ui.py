...
function render(){const x=current(),v=state.overrides[x.utterance_uid]||{};$('progress').textContent=`${index+1} / ${state.pending_count} - kaydedilen ${state.saved_count}`;$('uid').textContent=x.utterance_uid;$('kind').textContent=x.evidence_kind;$('before').textContent=x.context_before||'(boş)';$('after').textContent=x.context_after||'(boş)';$('evidence').textContent=[x.asr_text,x.youtube_text,x.secondary_transcript].filter(Boolean).join(' / ')||'(boş)';$('text').value=v.tr_corrected??x.tr_corrected??'';$('note').value=v.note||'';$('audio').src=`/audio/${encodeURIComponent(x.utterance_uid)}?token=${encodeURIComponent(token)}`;$('choices').replaceChildren();const choices=x.evidence_kind==='speech_hole'?['confirmed_dialogue','reviewed_non_dialogue']:['confirmed_dialogue','discarded_asr_hallucination'];for(const choice of choices){const b=document.createElement('button');b.textContent=choice;b.className=choice==='confirmed_dialogue'?'primary':'';b.onclick=()=>save(choice);$('choices').appendChild(b)}$('message').textContent=v.disposition?`Kaydedildi: ${v.disposition}`:'';$('message').className=v.disposition?'saved':''}
