"""Aligned-Turkish schema construction for the correction-first flow."""
...
def build_aligned_turkish_schema(
    ...
    *,
    ...
    for position, source in enumerate(segmented_blocks, start=1):
        ...
        try:
            current_speaker_id = speaker_id(source, f"segmented block {position}")
        except ValueError as exc:
            raise SchemaV2Error(str(exc)) from exc
        ...
        if start_ms < 0 or end_ms <= start_ms or unsafe_overlap:
        ...
        if current_speaker_id is not None:
            block["speaker_id"] = current_speaker_id
        blocks.append(block)
