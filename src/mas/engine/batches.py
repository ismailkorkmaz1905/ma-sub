...
def _default_glossary() -> dict[str, Any]:
    config_dir = Path(__file__).resolve().parents[3] / "config" / "production"
    names_path = config_dir / "names.yaml"
    religious_path = config_dir / "religious_terms.yaml"
...
def _resolve_instructions(
    ...
    path = (
        ...
        else Path(__file__).resolve().parents[3] / "config" / "production" / "TRANSLATION_INSTRUCTIONS.md"
    )
    if not path.is_file():
