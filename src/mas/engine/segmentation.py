"""Deterministic subtitle segmentation from forced-aligned timed words.
...
reordering words.  ``schema_v2.py`` remains the sole authority that fills stable
...
SEGMENTATION_FORMAT_VERSION = "2.0"

_REJECTED_CANONICAL_TIMING_SOURCES = frozenset(
    {
        # These timestamps are interpolated only to keep a decoder segment's
        # text serialisable.  They are not acoustic word alignment and must
        # never become immutable subtitle timing.
        "synthetic_segment_word_repair",
    }
)
_REPAIR_WORD_TIMING_SOURCES = frozenset(
    {
        "synthetic_segment_word_repair",
        "split_combined_whisper_word",
        "dotted_initialism_surface_repair",
    }
)
_NATIVE_WORD_TIMING_SOURCE = "native_word_timestamp"
...
class SegmentationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.preferred_cps <= 0 or self.maximum_source_cps < self.preferred_cps:
            raise ValueError("subtitle CPS bounds are invalid")
        ...
        if not (
            0 < self.sentence_gap_ms <= self.weak_phrase_gap_ms
            <= self.likely_speaker_gap_ms <= self.hard_silence_ms
        ):
            raise ValueError(
                "gap thresholds must satisfy sentence <= weak phrase <= "
                "likely speaker <= hard silence"
            )
...
def _prepare_words(source_words: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for offset, source in enumerate(source_words):
        ...
        try:
            current_speaker_id = speaker_id(source, f"word at offset {offset}")
        except ValueError as exc:
            raise SegmentationError(str(exc)) from exc
        active_words = [item for item in active_words if item[0] > start]
        if any(
            overlap_is_unsafe(prior_speaker, current_speaker_id)
            for _, prior_speaker in active_words
        ):
            raise SegmentationError(
                "Same/unknown-speaker overlapping word intervals are not canonical "
                f"alignment at offset {offset}"
            )
        if not text.strip():
            raise SegmentationError(f"Empty word text at offset {offset}")
        timing_source = str(source.get("timing_source", "")).strip()
        if timing_source in _REJECTED_CANONICAL_TIMING_SOURCES:
            raise SegmentationError(
                "Non-acoustic repaired timing cannot be canonical subtitle input: "
                f"offset {offset} timing_source={timing_source}"
            )
        alignment_sha256 = str(source.get("alignment_sha256", "")).strip().lower()
        if alignment_sha256:
            alignment_digests.add(alignment_sha256)
            if len(alignment_digests) > 1:
                raise SegmentationError(
                    "Canonical words reference multiple alignment_sha256 digests: "
                    + ", ".join(sorted(alignment_digests))
                )
        ...
        if current_speaker_id is not None:
            prepared["speaker_id"] = current_speaker_id
        if "utterance_uid" in source:
            prepared["utterance_uid"] = source["utterance_uid"]
        if timing_source:
            prepared["timing_source"] = timing_source
        if alignment_sha256:
            prepared["alignment_sha256"] = alignment_sha256
        words.append(prepared)
        previous_start = start
        active_words.append((end, current_speaker_id))
    return words
...
def _word_vad_memberships(
    ...
    for word in words:
        ...
        for fallback_index, region in enumerate(vad_regions, start=1):
            try:
                region_start = int(region["start_ms"])
                region_end = int(region["end_ms"])
            except (KeyError, TypeError, ValueError):
                continue
            if start_ms < region_end and region_start < end_ms:
                try:
                    region_id = int(
                        region.get("vad_region_index", fallback_index)
                    )
                except (TypeError, ValueError):
                    region_id = fallback_index
                region_ids.add(region_id)
        memberships.append(frozenset(region_ids))
    return memberships
...
def _hard_boundaries(
    ...
    vad_memberships = _word_vad_memberships(words, vad_regions)
    for offset in range(1, len(words)):
        ...
        reasons: list[str] = []
        speaker_risk = False
        # Decoder segment IDs describe a model implementation detail, not an
        # acoustic or speaker boundary.  In particular, a decoder may emit a
        # 19 ms one-word segment between two otherwise continuous phrases.  A
        # hard boundary is therefore based only on observable pause/turn cues.
        if gap >= config.hard_silence_ms:
            reasons.append("hard_silence")
        ...
        if "\n" in _word_text(previous) and _starts_dialogue_turn(_word_text(current)):
            ...
            speaker_risk = True
        previous_vad = vad_memberships[offset - 1]
        current_vad = vad_memberships[offset]
        if previous_vad and current_vad and previous_vad.isdisjoint(current_vad):
            # A decoder-independent acoustic boundary: words in two distinct
            # Silero islands must never share a cue, even when the aligned gap
            # is just below the generic likely-speaker threshold.
            reasons.append("vad_region_transition")
        # Provisional V2 acoustic-gap policy.  Decoder segment IDs remain
        # irrelevant, but an aligned pause must not hide a later word inside a
        # cue which appeared before that word was spoken.  Existing config
        # semantics define the thresholds: sentence punctuation at 180 ms,
        # clause/weak punctuation at 350 ms, and any pause at the 650 ms
        # likely-speaker threshold.  Each is a hard chunk boundary before DP.
        if gap >= config.likely_speaker_gap_ms:
            reasons.append("likely_speaker_gap")
            speaker_risk = True
        elif _ends_sentence(_word_text(previous)) and gap >= config.sentence_gap_ms:
            reasons.append("sentence_pause")
        elif _ends_clause(_word_text(previous)) and gap >= config.weak_phrase_gap_ms:
            reasons.append("weak_phrase_pause")
        if reasons:
            boundaries.append(
...
def _required_cue_duration_ms(text: str, config: SegmentationConfig) -> int:
    reading_duration_ms = math.ceil(len(text) / config.maximum_source_cps * 1000.0)
    return max(config.minimum_duration_ms, reading_duration_ms)
...
def _candidate_cost(
    ...
    *,
    ...
    next_start_ms = (
        int(words[end]["start_ms"])
        if end < len(words)
        else following_start_ms
    )
    required_end_ms = max(
        int(candidate[-1]["end_ms"]),
    ...
    if next_start_ms is not None:
        ...
        # make this split infeasible and force the DP to coalesce the words.
        latest_end_ms = min(latest_end_ms, next_start_ms - 1)
    if required_end_ms > latest_end_ms:
        return None

    display_end_ms = min(
        latest_end_ms,
        max(int(candidate[-1]["end_ms"]) + config.end_padding_ms, required_end_ms),
    )
    display_duration_ms = display_end_ms - cue_start_ms

    expected_ms = max(
        config.preferred_min_duration_ms,
        min(config.maximum_duration_ms - 500, round(character_count / config.preferred_cps * 1000)),
    )
    duration_cost = ((display_duration_ms - expected_ms) / 1000.0) ** 2
...
def _group_words(
    ...
    boundary_map = {
        int(words[boundary.before_word_offset]["source_offset"]): boundary
        for boundary in boundaries
    }
    chunk_ranges: list[tuple[int, int]] = []
    start = 0
    for boundary in boundaries:
        chunk_ranges.append((start, boundary.before_word_offset))
        start = boundary.before_word_offset
    chunk_ranges.append((start, len(words)))
    groups: list[list[dict[str, Any]]] = []
    for chunk_start, chunk_end in chunk_ranges:
        chunk = words[chunk_start:chunk_end]
        if chunk:
            following_start_ms = (
                int(words[chunk_end]["start_ms"]) if chunk_end < len(words) else None
            )
            groups.extend(
                _dynamic_groups(
                    chunk,
...
def _vad_info(
    ...
    decoder_segment_ids: list[Any] = []
    decoder_segment_crossings = 0
    previous_segment_id: Any = None
    have_previous_segment_id = False
    word_timing_sources: set[str] = set()
    alignment_digests: set[str] = set()
    missing_alignment_digest_count = 0
    repaired_word_count = 0
    for word in group:
        segment_id = word.get("segment_id")
        if segment_id is not None and segment_id not in decoder_segment_ids:
            decoder_segment_ids.append(segment_id)
        if (
            have_previous_segment_id
            and segment_id is not None
            and previous_segment_id is not None
            and segment_id != previous_segment_id
        ):
            decoder_segment_crossings += 1
        if segment_id is not None:
            previous_segment_id = segment_id
            have_previous_segment_id = True
        timing_source = str(word.get("timing_source", "")).strip()
        word_timing_sources.add(timing_source or _NATIVE_WORD_TIMING_SOURCE)
        if (
            timing_source in _REPAIR_WORD_TIMING_SOURCES
            or timing_source.startswith("synthetic_")
            or timing_source.endswith("_repair")
        ):
            repaired_word_count += 1
        alignment_sha256 = str(word.get("alignment_sha256", "")).strip().lower()
        if alignment_sha256:
            alignment_digests.add(alignment_sha256)
        else:
            missing_alignment_digest_count += 1
    if len(alignment_digests) > 1:
        raise SegmentationError(
            "A subtitle group references multiple alignment_sha256 digests: "
            + ", ".join(sorted(alignment_digests))
        )
    alignment_sha256 = next(iter(alignment_digests), None)
    alignment_risk_flags: list[str] = []
    if decoder_segment_crossings:
        alignment_risk_flags.append("decoder_segment_crossing")
    if repaired_word_count:
        alignment_risk_flags.append("repaired_word_provenance")
    if alignment_sha256 is not None and missing_alignment_digest_count:
        alignment_risk_flags.append("partial_alignment_digest_provenance")
    return {
        ...
        "alignment_provenance": {
            "canonical_word_intervals": True,
            "word_interval_policy": "strict_nonnegative_ordered_nonoverlap",
            "word_timing_sources": sorted(word_timing_sources),
            "repaired_word_count": repaired_word_count,
            "alignment_sha256": alignment_sha256,
            "missing_alignment_digest_word_count": missing_alignment_digest_count,
            "decoder_segment_ids": decoder_segment_ids,
            "decoder_segment_crossing_count": decoder_segment_crossings,
        },
        "alignment_risk_flags": alignment_risk_flags,
    }
...
def _risk_flags(
    ...
    if int(vad_info.get("max_internal_gap_ms", 0)) >= config.likely_speaker_gap_ms:
        flags.add("internal_gap_at_or_above_likely_speaker_threshold")
...
def _assign_timings(
    ...
    for index, block in enumerate(raw_blocks):
        ...
        text = str(block.get("timing_text", ""))
        required_end = start + _required_cue_duration_ms(text, config)
        desired_end = max(last_word_end + config.end_padding_ms, required_end)
        next_start = (
        ...
        if next_start is not None:
            ...
            if guarded_end >= max(last_word_end, required_end):
...
def build_blocks(
    ...
    *,
    ...
    explicit_speakers = {word.get("speaker_id") for word in words}
    if len(explicit_speakers) > 1:
        groups: list[list[dict[str, Any]]] = []
        boundary_map: dict[int, _Boundary] = {}
        for current_speaker in sorted(explicit_speakers, key=lambda value: str(value or "")):
            lane_words = [
    ...
    for group in groups:
        ...
        risk_flags.update(vad_info.get("alignment_risk_flags") or [])
    ...
    if None not in explicit_speakers and len(explicit_speakers) > 1:
        for current_speaker in sorted(explicit_speakers):
            _assign_timings(
    ...
    *,
    ...
    for expected_index, block in enumerate(blocks, start=1):
        ...
        if duration_ms < settings.minimum_duration_ms:
            invalid_timing_count += 1
            errors.append(
                f"{prefix}: duration {duration_ms} ms is below required minimum "
                f"{settings.minimum_duration_ms} ms"
            )
        try:
            current_speaker_id = speaker_id(block, prefix)
        except ValueError as exc:
        ...
        if any(
            ...
            overlap_count += 1
            errors.append(f"{prefix}: same/unknown-speaker overlap")
        active_blocks.append((end, current_speaker_id))
        ...
        if max_gap >= settings.likely_speaker_gap_ms:
            internal_gap_count += 1
            errors.append(
                f"{prefix}: unresolved internal gap {max_gap} ms is at or above "
                f"the hard {settings.likely_speaker_gap_ms} ms threshold"
            )
        ...
        if duration_ms > 0:
            ...
            if source_cps > settings.maximum_source_cps + 1e-9:
                invalid_timing_count += 1
                errors.append(
                    f"{prefix}: source CPS {source_cps:.3f} exceeds hard maximum "
                    f"{settings.maximum_source_cps:g}"
                )
