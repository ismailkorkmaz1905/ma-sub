...
def set_stage(path,state,name,status,**details):
 if status not in {'pending','running','pass','blocked','failed'}: raise ValueError(f'invalid stage status: {status}')
 state.setdefault('stages',{})[name]={'status':status,'updated_at':datetime.now(timezone.utc).isoformat(),**details}; save(path,state)
