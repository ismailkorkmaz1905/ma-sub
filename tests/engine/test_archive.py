...
from mas.engine.archive import (
    ArchiveV2Error,
    archive_episode_v2,
...
class ArchiveV2Tests(unittest.TestCase):
    ...
    def _populate_v2(self, root: Path) -> dict[str, Path]:
        ...
        config_root = project / "config" / "production"
    ...
    def _media_patches(self):
        return (
            patch("mas.engine.archive.compute_av_stream_hashes", return_value=self._stream_hashes()),
            patch(
                "mas.engine.archive.verify_mkv_roundtrip",
                return_value={"exact": True, "id_block_count": 1, "tr_block_count": 1},
            ),
            patch("mas.engine.archive.mux_softsubs", side_effect=self._fake_mux),
        )
    ...
    def test_verification_failure_never_removes_source_work_or_sidecars(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch("mas.engine.archive.compute_av_stream_hashes", return_value=self._stream_hashes()),
                patch("mas.engine.archive.verify_mkv_roundtrip", return_value={"exact": False}),
            ):
