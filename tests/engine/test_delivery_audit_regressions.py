...
def prepared(synthetic_parents, monkeypatch):
    ...
    atomic_json(root / 'source/official-source.json', {
        'episode': 15, 'title': 'Muhtemel Ask 15. Bolum', 'url': 'https://example.invalid/15',
    ...
    return root, dict(root=root, episode=15, source_video=source, audio_path=audio,
...
def _ready(prepared):
    ...
    schema = df.read_signed(child / 'work/delivery-schema.json', 'schema')['schema']
    ...
    return root, child, pf.validate_partial_export(root, 15, 'part-001')[0]
...
def test_no_dialogue_scope_reaches_authenticated_export_without_translation_handoff(prepared):
    ...
    decoded['segments'] = []
    assert df.run_worker(**options) == 26
    export, report = pf.validate_partial_export(root, 15, 'part-001')
    ...
    assert df.run_worker(**options) == 26
    assert pf.validate_partial_export(root, 15, 'part-001')[0] == export
    with pytest.raises(it.IDTranslationError):
        it.validate_aligned_turkish_schema({'schema_version': '2.0', 'episode': 0, 'blocks': []})
...
def test_recovery_uses_owned_audio_result_and_reuses_completed_jobs(tmp_path):
    ...
    binding = {'episode': 15, 'plan_sha256': 'p', 'duration_ms': 4000,
...
def test_recovery_deadline_and_ordinary_job_cap_do_not_block_delivery(tmp_path):
    child = tmp_path / 'parts/part-001'
    binding = {'episode': 15, 'plan_sha256': 'p', 'duration_ms': 4000, 'primary_producer': {'model': {}}}
    def forbidden(*a, **kw):
    ...
    df.write_signed(tmp_path / 'work/delivery-rescue-budget.json', {
        'episode': 15, 'plan_sha256': 'p', 'limit_seconds': dc.TOTAL_SECONDS,
...
def test_rehashed_controller_ack_cannot_skip_first_part(prepared):
    ...
    delivery = {'format': 'mas-part-delivery-1', 'status': 'PASS_PARTIAL', 'episode': 15,
        ...
        'mp4': {'relative_path': 'parts/part-001/output/fake.mp4', 'size_bytes': 1, 'sha256': '0' * 64},
        'files': [{'remote': 'drive:x.part-001.id.mp4', 'bytes': 1, 'sha256': '0' * 64}]}
    ack = {'format': 'mas-controller-part-ack-1', 'episode': 15, 'part_id': 'part-001',
    ...
    with pytest.raises(ValueError, match='authentication'):
        pd.validate_worker_published_part(root, 15, 'part-001')
...
def test_part_publication_and_encoding_reject_unsigned_self_asserted_receipts(prepared):
    ...
    atomic_json(child / 'output/partial-encoding.json', {'format': 'mas-partial-burned-id-mp4-1'})
    with pytest.raises(ValueError, match='authentication'):
        pe.validate_partial_encoding(root, 15, 'part-001')
    value = {'format': 'mas-part-delivery-1', 'status': 'PASS_PARTIAL'}
    atomic_json(child / 'output/drive_readback_receipt.json', {'data': value, 'sha256': digest(value)})
    with pytest.raises(ValueError, match='authentication'):
        pd.validate_published_part(root, 15, 'part-001')
...
def test_real_delivery_export_authorizes_collection_retry_not_forged_export(prepared):
    ...
    assert rc._collection_failure_paths_match(failure, {'episode': 15}, status)
    export['files'] = []
    atomic_json(root / 'work/partial-export.json', export)
    assert not rc._collection_failure_paths_match(failure, {'episode': 15}, status)


def test_expired_compute_budget_permits_only_retained_local_delivery(prepared, monkeypatch):
    root, child, _ = _ready(prepared)
    budget = {'episode': 15, 'started_at': '2020-01-01T00:00:00+00:00', 'limit_seconds': 21600}
    ...
    assert rc._resume_partial_delivery(root, 15) == rc.NEXT_PART
...
def test_continuous_speech_or_empty_vad_cannot_deadlock_first_hour(monkeypatch):
    ...
    audio = {'relative_path': 'work/audio.flac', 'sha256': '2' * 64, 'size_bytes': 1,
    ...
    plan = ps.build_part_plan(episode=15, source=source, audio=audio, vad=vad,
    ...
    with pytest.raises(ps.PartScopeError):
        ps.build_part_plan(episode=0, source=source, audio=audio, vad=vad)
    vad['regions'] = []
    empty = ps.build_part_plan(episode=15, source=source, audio=audio, vad=vad,
                              boundary_policy=ps.DELIVERY_BOUNDARY_POLICY)
...
def test_default_worker_recovers_detected_gap_then_freezes_real_id_pack(prepared, monkeypatch):
    ...
    assert df.run_worker(**options) == 26
    _, report = pf.validate_partial_export(root, 15, 'part-001')
    assert report['block_count'] == 1 and len(RescueSession.calls) == 1
    assert not (root / 'parts/part-002/work/delivery-primary.json').exists()
...
def test_changed_primary_identity_is_not_signed_as_current_result(prepared):
    ...
    with pytest.raises(ValueError, match='producer changed'):
        df.run_worker(**options)
    assert not (root / 'parts/part-001/work/delivery-primary.json').exists()
