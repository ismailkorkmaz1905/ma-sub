...
from mas.engine.episode_archive import (
    ArchiveError,
    archive_episode,
...
class EpisodeArchiveTests(unittest.TestCase):
    ...
    def test_episode_root_symlink_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self._symlink_or_skip(root, actual, target_is_directory=True)
    ...
    def test_project_ancestor_and_dangling_receipt_symlinks_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self._symlink_or_skip(linked_project, actual, target_is_directory=True)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self._symlink_or_skip(receipt, receipt.parent / "missing-receipt.json")
    ...
    def test_invalid_present_report_never_falls_back_to_receipt(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root, receipt = self._workspace(temporary)
            paths = self._populate(root)
            with patch("mas.engine.episode_archive.ensure_verified_mkv", side_effect=self._fake_ensure):
    ...
    def test_successful_cleanup_keeps_only_mkv_and_srts(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            paths = self._populate(root)
            with (
                patch("mas.engine.episode_archive.ensure_verified_mkv", side_effect=self._fake_ensure),
                patch("mas.engine.episode_archive._verify_ready_artifacts", return_value=None),
