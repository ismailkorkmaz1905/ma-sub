...
from mas import local_encode, notify
...
def test_subtitle_ready_exit_collects_only_strict_subtitle_export(tmp_path, monkeypatch):
    ...
    controller._collect_remote_results(24, 14, tmp_path, "/workspace/ma-sub/EPISODES/Muhtemel Ask 14.Bolum",
...
def test_encode_resume_never_requires_gpu_environment_or_acquires_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(notify, "drain_outbox", lambda *a, **kw: events.append("drain"))
    assert controller.run_remote_episode(14) == 0
    assert events == ["local-encode", "validate", "delivery-release", "publish", "drain"]
...
def test_encode_failure_preserves_resume_boundary_without_acquiring_gpu(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="QSV unavailable"):
        controller.run_remote_episode(14)
    assert (tmp_path / "work/gpu-released-for-encode.json").exists()


def test_mail_drain_waits_for_verified_capacity_release_even_on_failure(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        with controller._drain_after_capacity(tmp_path, 14, audit):
            events.append("work")
            raise RuntimeError("failure")
    assert events == ["work", "verified-absent", "drain"]
