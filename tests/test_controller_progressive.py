...
def _handoff(root, kind='tr'):
    ...
    pack = child / 'translation_input' / (name + ('_TR_CORRECTION_PACK.zip' if kind == 'tr' else '_ID_TRANSLATION_PACK.zip'))
    output = child / 'translation_output' / (name + ('_TR_TEXT_CORRECTED.zip' if kind == 'tr' else '_ID_TRANSLATED.zip'))
    pack.parent.mkdir(parents=True, exist_ok=True)
    pack.write_bytes(kind.encode())
    return progressive.write_partial_handoff(root, 14, 'part-001', kind, pack, output, [pack])
...
def _release(root, code):
    ...
    request = {'episode': 14, 'commit': 'a' * 40, 'input_sha256': 'b' * 64}
    ...
    atomic_json(root / 'work/remote-job-status.json', {'status': 'EXITED', 'exit_code': code,
                'identity': _identity(14, 'a' * 40, 'b' * 64)})
    return audit
...
def test_handoff_transport_keeps_frozen_phases_without_parent_media(tmp_path, monkeypatch):
    ...
    for kind in ('tr', 'id'):
        ...
        controller._collect_remote_results(WAIT_PART_RETURN, 14, local, '/episode', [], ['scp'],
                                           'host', Budget(), staging)
        assert progressive.validate_partial_handoff(local, 14)['kind'] == kind
...
def test_partial_transfer_rejects_crosspart_inventory(tmp_path, monkeypatch):
    ...
    data['files'][0]['relative_path'] = 'parts/part-002/prepare/evidence.json'
...
def test_missing_return_recovers_pause_after_verified_shutdown_without_gpu(tmp_path, monkeypatch):
    ...
    assert controller.run_remote_episode(14) == WAIT_PART_RETURN
...
def test_expired_part_wait_cannot_reset_six_hour_clock(tmp_path, monkeypatch):
    ...
    with pytest.raises(BudgetExceeded):
        controller._resume_partial_delivery(tmp_path, 14)


def _current_part(root, part_id='part-001', stage='forced_alignment'):
    lineage = root / f'parts/{part_id}/prepare/audio-part.done.json'
    ...
    atomic_json(state_path, {'episode': 14, 'part_id': part_id, **hashes,
                            'stages': {stage: {'status': 'failed'}}})
    data = {'format': 'mas-current-part-1', 'episode': 14, 'part_id': part_id, 'stage': stage,
...
def test_partial_failure_identity_is_exact_current_part_not_old_conflict(tmp_path):
    ...
    result = controller._partial_failure_identity(tmp_path, 14)
    ...
    assert controller._partial_failure_identity(tmp_path, 14) == {'part_id': 'part-002', 'part_stage': 'id_return'}
...
def test_partial_failure_rejects_changed_state_with_rehashed_current_pointer(tmp_path):
    ...
    with pytest.raises(controller.RunPodControllerError, match='audio evidence changed'):
        controller._partial_failure_identity(tmp_path, 14)
...
def test_new_job_failure_cannot_adopt_previous_job_alignment_marker(tmp_path):
    ...
    current['remote_job_token'] = _identity(14, 'a' * 40, 'b' * 64)['token']
    _bound(tmp_path / 'work/current-part.json', current)
    assert controller._partial_failure_identity(tmp_path, 14)['part_stage'] == 'forced_alignment'
...
def test_partial_diagnostics_only_snapshot_active_part(tmp_path):
    ...
    for part_id in ('part-001', 'part-002'):
        atomic_json(root / f'parts/{part_id}/prepare/forced_alignment_units/resume-identity.json', {'part': part_id})
        atomic_json(root / f'parts/{part_id}/prepare/raw_asr_v2.json', {'fixture': part_id})
    manifest = checkpoint_manifest(tmp_path, 14, 'a' * 40, diagnostics=True)
    ...
    assert 'parts/part-001/prepare/forced_alignment_units/resume-identity.json' in relatives
    assert 'parts/part-001/prepare/raw_asr_v2.json' in relatives
...
def test_worker_ack_only_completion_cannot_start_another_gpu_loop(tmp_path, monkeypatch):
    ...
    with pytest.raises(controller.RunPodControllerError, match='no-progress GPU continuation'):
        controller._collect_remote_results(NEXT_PART, 14, tmp_path, '/episode', [], [], 'host', Budget(), tmp_path)


def test_partial_collection_retry_matches_plain_export(tmp_path):
    relative = 'parts/part-001/final/strict.srt'
    atomic_json(tmp_path / 'work/partial-export.json', {'episode': 14, 'mode': 'strict-partial-subtitles',
...
def test_part_review_uploads_keep_namespace_and_do_not_authorize_unvalidated_retry(tmp_path):
    ...
    for filename in ('audio_review_overrides.json', 'speaker_evidence_v1.json'):
        atomic_json(tmp_path / 'parts/part-001/review' / filename, {'fixture': 'unvalidated'})
    paths = controller._part_resume_files(tmp_path, 14)
    assert {path.relative_to(tmp_path).as_posix() for path in paths} == {
        'parts/part-001/review/audio_review_overrides.json', 'parts/part-001/review/speaker_evidence_v1.json'}
    after, evidence_after = controller._remote_input_binding(tmp_path, 'source', 'a' * 40, 14)
    assert before != after
...
@pytest.mark.parametrize('capacity', ['none', 'active', 'released', 'tampered'])
def test_cli_mail_never_drains_while_owned_compute_is_active(tmp_path, monkeypatch, capacity):
    monkeypatch.setattr(controller, 'ROOT', tmp_path)
    monkeypatch.setattr(controller, 'episode_dir', lambda _: tmp_path)
    calls = []
    monkeypatch.setattr(controller, '_drain_notifications', lambda *_: calls.append('SMTP'))
    if capacity != 'none':
        audit = _release(tmp_path, 1)
        if capacity == 'active':
            _bound(audit / 'capacity-state.json', {'episode': 14, 'status': 'RUNNING', 'owned_pod_ids': ['owned']})
        elif capacity == 'tampered':
            atomic_json(audit / 'capacity-shutdown.json', {'forged': True})
    controller.drain_cli_notifications(14)
    assert calls == (['SMTP'] if capacity in {'none', 'released'} else [])
...
def test_first_hour_dispatch_precedes_full_episode_asr(tmp_path, monkeypatch, valid_budget):
    ...
    monkeypatch.setattr(pipeline, '_resolve_source_url', lambda *a: 'https://example.invalid/14')
    ...
    controller._episode_budget(tmp_path, 14)
    if not valid_budget:
        atomic_json(tmp_path / 'work/controller_budget.json', {'data': {'episode': 14, 'limit_seconds': 21601}})
        with pytest.raises(RuntimeError, match='original bounded episode clock'):
            pipeline.run(14)
        assert not calls
    else:
        assert pipeline.run(14) == WAIT_PART_RETURN
        assert len(calls) == 1
