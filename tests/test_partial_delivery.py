...
def bridge(tmp_path, monkeypatch):
    ...
    atomic_json(root / 'source/official-source.json', {'episode': 12, 'url': 'https://example.invalid/12',
                'channel_url': delivery.CHANNEL_VIDEOS_URL, 'title': 'Muhtemel Ask 12. Bolum'})
    ...
    state = {'format': 'mas-capacity-lease-state-1', 'episode': 12, 'status': 'RELEASED',
    ...
    delivery.write_part_release(root, 12, 'part-001', 'owned', audit)
    uploads = []
...
def test_bridge_emits_only_partial_receipt_ack_and_explicit_parts_completion(bridge):
    ...
    assert delivery.complete_local_part(root, 12, 'part-001', 'drive:strict', total_timeout=120) == delivery.NEXT_PART
    published = delivery.validate_published_part(root, 12, 'part-001')
    assert published['status'] == 'PASS_PARTIAL' and len(uploads) == 1
    assert delivery.validate_worker_published_part(root, 12, 'part-001')['delivery'] == published
    completed = delivery.complete_parts(root, 12)
...
def test_bad_upload_result_never_leaves_partial_pass_or_ack(bridge, monkeypatch, failure):
    ...
    with pytest.raises((ValueError, RuntimeError)):
        delivery.complete_local_part(root, 12, 'part-001', 'drive:strict', total_timeout=120)
    assert not (child / 'final/drive_readback_receipt.json').exists()
...
def test_cached_published_part_restores_ack_without_encoder_or_upload(bridge, monkeypatch):
    ...
    assert delivery.complete_local_part(root, 12, 'part-001', 'drive:strict', total_timeout=120) == delivery.NEXT_PART
    assert delivery.validate_worker_published_part(root, 12, 'part-001') is not None


def test_cached_publication_cannot_claim_a_different_destination(bridge, monkeypatch):
    root, child, _, _ = bridge
    delivery.complete_local_part(root, 12, 'part-001', 'drive:strict', total_timeout=120)
    before = (child / 'final/drive_readback_receipt.json').read_bytes()
    monkeypatch.setattr(partial_encode, 'burn_partial_indonesian_mp4', lambda *a, **kw: pytest.fail('re-encode'))
    with pytest.raises(ValueError, match='destination differs'):
        delivery.complete_local_part(root, 12, 'part-001', 'drive:different', total_timeout=120)
    assert (child / 'final/drive_readback_receipt.json').read_bytes() == before
...
def test_worker_ack_needs_no_mp4_but_local_completion_requires_it(bridge):
    root, child, _, _ = bridge
    delivery.complete_local_part(root, 12, 'part-001', 'drive:strict', total_timeout=120)
    receipt = delivery.validate_published_part(root, 12, 'part-001')
    (root / receipt['mp4']['relative_path']).unlink()
    assert delivery.validate_worker_published_part(root, 12, 'part-001') is not None
    with pytest.raises(ValueError):
        delivery.complete_parts(root, 12)
...
def test_later_part_failure_preserves_earlier_delivery(bridge, monkeypatch):
    ...
    with pytest.raises((FileNotFoundError, ValueError)):
        delivery.complete_local_part(root, 12, 'part-002', 'drive:strict', total_timeout=120)
    assert (child / 'final/drive_readback_receipt.json').read_bytes() == before
    assert delivery.validate_published_part(root, 12, 'part-001')['status'] == 'PASS_PARTIAL'
...
def test_expired_shared_budget_never_writes_delivery_receipt(bridge, monkeypatch):
    ...
    with pytest.raises(TimeoutError):
        delivery.complete_local_part(root, 12, 'part-001', 'drive:strict', total_timeout=1)
    assert not (child / 'final/drive_readback_receipt.json').exists()
