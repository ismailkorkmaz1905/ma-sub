...
def disable_delivery_notifications(monkeypatch):
    monkeypatch.setattr(delivery, 'notify', lambda *args, **kwargs: {'status': 'disabled'})
...
def _strict_delivery(root, episode=13, with_samples=False):
    name = f'Muhtemel Ask {episode}.Bolum'
    inputs = {
        'source_video': _record(root, 'source/source.mp4', b'source'),
        'tr_correction_output': _record(root, 'translation_output/tr.zip', b'tr return'),
        'id_translation_output': _record(root, 'translation_output/id.zip', b'id return'),
    }
    outputs = {
        'tr_srt': _record(root, f'final/{name}.tr.srt', b'tr srt'),
        'id_srt': _record(root, f'final/{name}.id.srt', b'id srt'),
        'mkv': _record(root, f'final/{name}.mkv', b'mkv'),
    }
    ...
    mp4 = _record(root, f'final/{name}.id.bound.mp4', b'mp4')
    ...
    if with_samples:
        ...
        approval_path = root / 'review' / 'mp4-sample-approval.json'
    ...
    manifest = {'mode': 'strict', 'strict_finalization_sha256': delivery.sha256_file(report_path),
                ...
                'outputs': {'mp4': mp4}}
    atomic_json(root / 'final' / 'burned_mp4_delivery.json', manifest)
...
def test_wait_23_transfers_only_samples_and_does_not_acquire_gpu(tmp_path, monkeypatch):
    ...
    atomic_json(sample_export, {'episode': 13, 'mode': 'review', 'files': []})
    ...
    runpod_controller._episode_budget(episode_root, 13)
    runpod_controller._pause_episode_budget(episode_root, 13, delivery.WAIT_MP4_SAMPLE,
                                           sample_export, shutdown)
    monkeypatch.setattr(runpod_controller, 'CapacityProvider',
                        lambda *args: pytest.fail('sample wait acquired GPU capacity'))
    assert runpod_controller.run_remote_episode(13) == delivery.WAIT_MP4_SAMPLE
    ...
    monkeypatch.setattr(pipeline, 'notify', lambda *args, **kwargs: None)
    result = pipeline._stage(tmp_path / 'state.json', {'episode': 13}, 'finalize',
...
def test_external_mp4_export_is_exact_and_downloads_to_canonical_local(tmp_path, monkeypatch):
    ...
    data = b'external mp4'
    record = {'relative_path': 'final/Muhtemel Ask 13.Bolum.id.bound.mp4',
              'storage_path': '/tmp/mas-ep13-output/Muhtemel Ask 13.Bolum.id.bound.mp4',
    ...
    delivery_record = dict(record, storage_path=str(Path('/tmp/mas-ep13-output') / canonical.name))
    assert delivery.remote_storage_path(local_root, delivery_record, 13) == Path(delivery_record['storage_path'])
    with pytest.raises(ValueError, match='unexpected external'):
        delivery.remote_storage_path(local_root, dict(delivery_record, storage_path='elsewhere'), 13)
    bad = dict(record, storage_path='/tmp/mas-ep12-output/' + canonical.name)
...
def test_strict_source_returns_subtitles_and_mkv_remain_hash_bound(tmp_path):
    ...
    delivery.validate_delivery(tmp_path, 13)
...
def test_remote_inventory_preservation_mismatch_aborts_publication(tmp_path, monkeypatch):
    ...
    atomic_json(tmp_path / 'work' / 'state.json', {'episode': 13, 'stages': {}})
    ...
    with pytest.raises(RuntimeError, match='retained Drive'):
        delivery.publish_local_delivery(tmp_path, 13, 'drive:delivery')
...
def test_mutated_approved_sample_blocks_local_delivery(tmp_path):
    ...
    delivery.validate_delivery(tmp_path, 13)
    (tmp_path / 'source' / 'source.url').write_text('https://example.test/13', encoding='utf-8')
    atomic_json(tmp_path / 'source' / 'official-source.json', {'episode': 13})
    export = delivery.write_delivery_export(tmp_path, 13)
    exported = {record['relative_path'] for record in export['files']}
    assert 'review/mp4-sample-approval.json' in exported
