...
def _case(tmp_path, monkeypatch, *, outcome="pass", bound=True):
    ...
    ledger = {"episode": 15, "started_at": start, "limit_seconds": 21600}
    atomic_json(root / "work/controller_budget.json", {"data": ledger, "sha256": digest(ledger)})
    request = {"episode": 15, "commit": "a" * 40, "input_sha256": "a" * 64}
    ...
    for relative in ("source/download.done.json", "work/audio.done.json", "work/raw_asr_v2.done.json",
    ...
    for relative in (
        ...
        "handoff/Muhtemel Ask 15.Bolum_TR_TEXT_CORRECTED.zip",
    ...
    identity_path = "work/forced_alignment_units/resume-identity.json"
    atomic_json(root / identity_path, {"data": identity, "sha256": digest(identity)})
    record = retry._record(root, identity_path)
    component_path = "work/forced_alignment_units/components/latest-conflict-failure.json"
...
def test_code_fix_authorization_requires_actual_bound_fixture_pass(tmp_path, monkeypatch):
    ...
    permit = retry.authorize_code_fix_retry(root, 15, "b" * 40, **arguments)
    assert retry.validate_code_fix_resume(root, 15, "b" * 40, permit) == arguments["resume_scope"]
    ...
    assert "work/raw_asr_v2.json" in predecessor_paths
    assert (
        "handoff/Muhtemel Ask 15.Bolum_TR_CORRECTED.zip"
...
def test_code_fix_permit_binds_bounded_discovered_component_limit(
    ...
    scope = retry.validate_code_fix_resume(root, 15, "b" * 40, permit)
...
def test_failed_or_skipped_fixture_never_authorizes(tmp_path, monkeypatch, outcome):
    ...
    with pytest.raises(retry.RetryAuthorizationError, match="pass|skipped"):
        retry.authorize_code_fix_retry(root, 15, "b" * 40, **arguments)
    assert not (root / "work/code-fix-resume.json").exists()
...
def test_permit_cannot_rebind_changed_evidence_or_restart_wall_clock(tmp_path, monkeypatch, changed):
    ...
    with pytest.raises((retry.RetryAuthorizationError, BudgetExceeded)):
        retry.validate_code_fix_resume(root, 15, commit)
...
def _part_case(tmp_path, monkeypatch):
    ...
    audio = {"relative_path": "work/audio.flac", "sha256": "f" * 64, "size_bytes": 100,
    ...
    plan = build_part_plan(episode=15, source={"relative_path": "source/movie.mp4", "sha256": "e" * 64, "size_bytes": 100},
    ...
    part = plan["parts"][0]
    lineage = {"format": "mas-derived-audio-part-1", "episode": 15, "part_id": part_id,
               ...
               **{key: part[key] for key in ("start_ms", "end_ms", "start_sample", "end_sample")},
               ...
               "audio": {"relative_path": f"parts/{part_id}/work/audio.flac", "sha256": "a" * 64, "size_bytes": 80},
               "producer_sha256": "2" * 64, "ffmpeg_identity": {"version": "fixture"}, "pcm_sha256": "3" * 64}
    atomic_json(child / "work/audio-part.done.json", {"data": lineage, "sha256": digest(lineage)})
    ...
    raw = {"format_version": "2.0", "status": "completed", "input_sha256": "4" * 64, "episode": 15,
    ...
    write_stage_marker(child / "work/raw_asr_v2.done.json", stage="raw_asr_v2", input_sha256=raw["input_sha256"],
                       outputs={"raw_asr_v2": raw_path}, details={"audio_sha256": "a" * 64})
    name = "Muhtemel Ask 15.Bolum"
    pack = child / "handoff" / f"{name}_TR_CORRECTION_PACK.zip"
    text = child / "handoff" / f"{name}_TR_TEXT_CORRECTED.zip"
    final = child / "handoff" / f"{name}_TR_CORRECTED.zip"
    create_tr_correction_pack(utterances, [], pack, episode=15)
    ...
    resolve_tr_audio_reviews_v2(pack, text, final, child / "work/audio_review_v2.json",
                                child / "work/audio_review_v2.recovery.json", progress=None)
    ...
    prefix = f"parts/{part_id}/work/forced_alignment_units/"
    ...
    scope = {**arguments["resume_scope"], "source_sha256": digest(source), "part_id": part_id,
             ...
             "part_audio_lineage_sha256": retry.sha256_file(child / "work/audio-part.done.json")}
...
def test_part_retry_validates_frozen_child_predecessors_without_parent_media(tmp_path, monkeypatch):
    ...
    permit = retry.authorize_code_fix_retry(root, 15, "b" * 40, **args)
    assert retry.validate_code_fix_resume(root, 15, "b" * 40, permit) == args["resume_scope"]
    ...
    assert "work/raw_asr_v2.done.json" not in paths
    assert "parts/part-001/work/raw_asr_v2.json" in paths
    assert "parts/part-001/work/audio-part.done.json" in paths
    assert not (root / "work/audio.flac").exists()
...
def test_part_retry_rejects_cross_scope_or_changed_predecessor(tmp_path, monkeypatch, change):
    ...
    if change in {"part", "stage"}:
        ...
        args["diagnostic_records"][0] = retry._record(root, "work/forced_alignment_units/resume-identity.json")
    else:
        relative = {"lineage": "work/audio-part.done.json", "raw": "work/raw_asr_v2.json",
                    "tr": "handoff/Muhtemel Ask 15.Bolum_TR_CORRECTED.zip"}[change]
    ...
    with pytest.raises((retry.RetryAuthorizationError, ValueError, RuntimeError)):
        retry.authorize_code_fix_retry(root, 15, "b" * 40, **args)
    assert not (root / "work/code-fix-resume.json").exists()
...
def test_same_failed_revision_cannot_receive_code_fix_permit(tmp_path, monkeypatch):
    ...
    with pytest.raises(retry.RetryAuthorizationError, match="unchanged failed revision"):
        retry.authorize_code_fix_retry(root, 15, "a" * 40, **args)


@pytest.mark.parametrize("relative,new", [
    ("work/raw_asr_v2.json", False),
    ("work/audio_review_v2.json", False),
    ("work/audio_review_overrides.json", True),
    ("work/speaker_evidence_v1.json", True),
])
def test_part_permit_rejects_changed_or_new_child_evidence(tmp_path, monkeypatch, relative, new):
    ...
    with pytest.raises(retry.RetryAuthorizationError, match="checkpoint|changed"):
        retry.validate_code_fix_resume(root, 15, "b" * 40, permit)
...
def test_offline_closure_proposal_is_not_a_gpu_permit(tmp_path, monkeypatch):
    ...
    path = retry.propose_alignment_recovery(root, 15, max_new_ctc_calls=4)
    body = retry._read_bound(path)
    ...
    permit = retry.authorize_code_fix_retry(root, 15, 'b' * 40, **arguments)
    assert retry.validate_code_fix_resume(root, 15, 'b' * 40, permit) == body['scope']
    with pytest.raises(retry.RetryAuthorizationError, match='proposal episode'):
        retry.propose_alignment_recovery(root, 16, max_new_ctc_calls=4)


def test_planning_cli_never_starts_provider_on_missing_evidence(tmp_path, monkeypatch, capsys):
    from mas import cli
    ...
    assert cli.main(['plan-alignment-recovery', '15', '--max-new-ctc-calls', '64']) == 1
